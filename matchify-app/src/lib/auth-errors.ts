...
const getGraphQLErrors = (error: unknown): unknown[] => {
...
const hasUnauthenticatedGraphQLError = (error: unknown) =>
  getGraphQLErrors(error).some((graphQLError) => {
    ...
    const message = 'message' in graphQLError ? graphQLError.message : undefined
    if (message === 'UNAUTHENTICATED') return true

    const extensions = 'extensions' in graphQLError ? graphQLError.extensions : undefined
    if (!extensions || typeof extensions !== 'object' || !('code' in extensions)) return false
